...
function TurbineSimulatorIcon() {
  const [toggle, setToggle] = useState(false);
  const blades = [
    <Blades key="blades" />
  ]
  const [speed, setvalue] = useState(50);
...
//const speed = getValue;
  let rotation = 60000/speed;
  let config = {duration: rotation};
  console.log(config);

  const springs = useSprings(blades.length, blades.map(()=> {
    return {
      cancel: toggle ? false : true,
      loop: true,
      from: { rotateZ: 0 },
      to: { rotateZ: 720 },
      config: config
  ...
  return(
    <div style={{ width:300, margin: 30, alignContent: "center" }}>
      <div>
  ...
  <Typography align='center'>
  <div style={{margin: 20}}>
      <Button variant="contained"  onClick={() => { setToggle(!toggle) }}>{speed} rpm</Button>
    </div>
    <div style={{margin: 20}}>
      <Slider direction="rtl" size= "large" color= "primary" defaultValue={50} aria-label="Default" valueLabelDisplay="auto" max={100} onChange={handleChange} />
    </div>
...
export default TurbineSimulatorIcon;
