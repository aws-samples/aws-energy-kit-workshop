...
class IoTTwinMakerUnifiedDataQuery(ABC):
    ...
    def process_query(self, lambda_event):
        from udq_utils.udq import SingleEntityReader, MultiEntityReader # pylint: disable=E0611:no-name-in-module
        ...
        if (isinstance(request, IoTTwinMakerUDQEntityRequest)):
            if isinstance(self, SingleEntityReader):
                udq_response = self.entity_query(request) # pylint: disable=E1101:no-member
            else:
            ...
            if isinstance(self, MultiEntityReader):
                udq_response = self.component_type_query(request) # pylint: disable=E1101:no-member
            else:
        ...
        def serialize_value(val):
            if type(val) is str:
                ...
                assert False # nosec - assertion is wrapped in conditional
